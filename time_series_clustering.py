...
print(f"   - {n_samples} time series with {X.shape[1]} timestamps each")
print(f"   - {n_classes} true classes")

print("\n2. Clustering Performance:")
best_method = methods[np.argmax(ari_scores)]
print(f"   - Best method based on ARI: {best_method} (ARI = {np.max(ari_scores):.4f})")
print(f"   - DTW K-means achieved an ARI of {ari_km_dtw:.4f}")
print(f"   - Hierarchical Clustering achieved an ARI of {ari_hc:.4f}")
print(f"   - KShape achieved an ARI of {ari_ks:.4f}")
print(f"   - DBSCAN achieved an ARI of {ari_dbscan:.4f}")

print("\n3. Key Findings:")
if ari_km_dtw > ari_hc and ari_km_dtw > ari_ks and ari_km_dtw > ari_dbscan:
    print("   - DTW K-means performed best, suggesting that the dataset's structure")
    print("     is well-suited for centroid-based clustering with DTW distance")
elif ari_hc > ari_km_dtw and ari_hc > ari_ks and ari_hc > ari_dbscan:
    print("   - Hierarchical clustering performed best, suggesting that the dataset")
    print("     has a nested structure that hierarchical methods can capture better")
elif ari_ks > ari_km_dtw and ari_ks > ari_hc and ari_ks > ari_dbscan:
    print("   - KShape performed best, suggesting that the shape patterns in the time")
    print("     series are more important than the absolute values for clustering")
else:
    print("   - DBSCAN performed best, suggesting that the dataset contains clusters")
    print("     of varying densities or non-globular shapes that density-based methods can capture better")

print("\n4. DBSCAN Specific Insights:")
print(f"   - DBSCAN identified {len(np.unique(y_dbscan))} clusters with epsilon={epsilon:.4f}")
if -1 in y_dbscan:
    noise_percent = np.sum(y_dbscan == -1) / len(y_dbscan) * 100
    print(f"   - {np.sum(y_dbscan == -1)} points ({noise_percent:.2f}%) were classified as noise")
    if noise_percent > 20:
        print("   - High noise percentage suggests either the need for parameter tuning or")
        print("     the presence of outliers/anomalies in the dataset")
    else:
        print("   - The reasonable noise percentage suggests DBSCAN effectively identified")
        print("     the core structure of the clusters")
else:
    print("   - DBSCAN did not identify any noise points, suggesting the data is well-structured")

print("\n5. Method Comparison:")
if ari_dbscan < min(ari_km_dtw, ari_hc, ari_ks):
    print("   - DBSCAN underperformed compared to other methods, which may indicate")
    print("     that the clusters in this dataset have similar densities and are well-separated")
else:
    print("   - DBSCAN's performance indicates that density-based approaches can be valuable")
    print("     for time series clustering, especially when clusters have varying densities")